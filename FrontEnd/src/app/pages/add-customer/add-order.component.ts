...
import { CustomerService, EntityCustomerResponseType } from 'src/app/services/orders.service';
...
export class AddOrderComponent implements OnInit {
    ...
    this.getCustomerByAccountNumber(accountNumber).subscribe((customer) => {
      if(customer){
        ...
        const formData = {
          ...
          currencyRate: this.customer.currencyRateFromSAR,
          status: this.customer.status,
          smsaServiceFee: this.customer.smsaServiceFromSAR,
          nameArabic: this.customer.nameArabic,
          nameEnglish: this.customer.nameEnglish,
          customerVAT: this.customer.vatNumber,
        ...
        this.customerForm.get('account')?.disable()
      }
    });
  }
  ...
  submit(customerForm: FormGroup) {
    ...
    let customer = {
      ...
      currencyRateFromSAR: customerForm.value.currencyRate,
    ...
    if(this.accountNumber!=null && this.accountNumber.length!=0){
      this.updateCustomer(this.accountNumber, customer);
    }else{
  ...
  updateCustomer(accountNumber: any, customer: Customer){
    this.customerService.updateCustomer(accountNumber, customer).subscribe((res: any)=>{
      if(res){
        this.router.navigateByUrl('/customer/view')
      }
    })
  ...
  getCustomerByAccountNumber(accountNumber?: any): Observable<Customer | null> {
    return this.customerService.getCustomerByAccountNumber(accountNumber).pipe(
      map((res: EntityCustomerResponseType) => {
        if (res && res.body) {
          return res.body;
        }
        return null;
